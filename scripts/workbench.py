...
def main():
    ...
    try:
        ...
        _CONTRACT_INIT = f"contracts/counter_init.avmbc"
        _CONTRACT_BODY = f"contracts/counter_runtime.avmbc"
    
        # Deploy a contract
        colored_print("\n5. Deploying contract...", Colors.BOLD)
        deploy_result = run_cmd(f"go run cmd/wallet/main.go --node localhost:8334 --privkey {privkey} deploy --init {_CONTRACT_INIT} --body {_CONTRACT_BODY} --amount 1000000 --gas-limit 50000 --gas-price 100", capture_output=False, output_color=Colors.RED)
        ...
        input("Press enter to call it first time...")
