...
type BlockCreationParams struct {
	...
	CurrentStates map[types.PublicKey]*types.AccountState // Current blockchain state for execution
}

// NewBlock creates a new block and mines it to satisfy targetBits
func NewBlock(params BlockCreationParams) (types.Block, error) {
	ts := params.Timestamp
	if ts == 0 {
	...
	tsxs := append([]types.Transaction{params.Coinbase}, params.Transactions...)

	// Actually execute transactions to get exact gas usage and fees
	// Create a copy of current chain state for execution
	tempAccountStates := make(map[types.PublicKey]*types.AccountState)
	for pubKey, state := range params.CurrentStates {
		if state != nil {
			tempAccountStates[pubKey] = &types.AccountState{
	...
	for _, tx := range params.Transactions {
		...
		if err != nil {
			return types.Block{}, fmt.Errorf("gas fees sum overflow: %w", err)
		}
		gasFeeSum = newGasFeeSum

		newTotalGasUsed, err := utils.SafeAdd(totalGasUsed, gasUsed)
		if err != nil {
			return types.Block{}, fmt.Errorf("total gas used overflow: %w", err)
		}
	...
	if err != nil {
		return types.Block{}, fmt.Errorf("coinbase amount calculation overflow: %w", err)
	}
	if params.Coinbase.Amount != expectedCoinbaseAmount {
		return types.Block{}, fmt.Errorf("Coinbase amount (%d) must equal BlockReward (%d) + gas fees (%d)",
			params.Coinbase.Amount, config.BlockReward, gasFeeSum)
	}

	// Calculate Merkle root
	merkleRoot := crypt.MerkleTransactions(tsxs)
	...
	if totalGasUsed > blockGasLimit {
		return types.Block{}, fmt.Errorf("transactions exceed block gas limit: used %d, limit %d", totalGasUsed, blockGasLimit)
	}
	...
	header := types.BlockHeader{
		...
		StateRoot:    types.Hash32{}, // TODO: Calculate proper state root
		ReceiptRoot:  types.Hash32{}, // TODO: Calculate proper receipt root
...
func MineCorrectNonce(header *types.BlockHeader, targetBits uint32) (NonceType, error) {
	for {
		hash := header.GetHash()
