...
// PublicKeyToArray converts ed25519.PublicKey to models.PublicKey
func PublicKeyToArray(pub ed25519.PublicKey) models.PublicKey {
...
func GenerateValidTransaction(from TestAccount, to models.PublicKey, amount, nonce uint64) models.Transaction {
	tx := models.Transaction{
		From:      PublicKeyToArray(from.PublicKey),
		To:        to,
		Amount:    amount,
		Nonce:     nonce,
		Signature: models.Signature{}, // Will be filled by signing
	}

	// Sign the transaction
	hashing.SignTransaction(&tx, from.PrivateKey)

	return tx
}

// GenerateCoinbaseTransaction creates a coinbase transaction (mining reward)
func GenerateCoinbaseTransaction(to models.PublicKey, amount uint64) models.Transaction {
	return models.Transaction{
		From:      models.PublicKey{}, // Zero/empty for coinbase
		To:        to,
		Amount:    amount,
...
func GenerateRandomTransactions(accounts []TestAccount, count int) []models.Transaction {
	if len(accounts) < 2 {
	...
	nonces := make(map[models.PublicKey]uint64)
...
func GenerateTestBlock(prevHash [32]byte, transactions []models.Transaction, timestamp uint64) models.Block {
	if timestamp == 0 {
		timestamp = uint64(time.Now().Unix())
	}

	merkleRoot := hashing.MerkleTransactions(transactions)
...
func GenerateMinedTestBlock(prevHash [32]byte, transactions []models.Transaction, timestamp uint64) models.Block {
	block := GenerateTestBlock(prevHash, transactions, timestamp)

	// Mine the block to find valid nonce
	nonce, err := mining.MineCorrectNonce(&block.Header)
	if err != nil {
...
func GeneratePrebuiltChain(blockCount int, accountCount int, transactionsPerBlock int) []*models.Block {
	accounts := GenerateTestAccounts(accountCount)
	firstUser := GetFirstUserTestAccount()
	blocks := make([]*models.Block, 0, blockCount+2) // +2 for genesis and distribution block

	// Global nonce tracking across all blocks
	globalNonces := make(map[models.PublicKey]uint64)
	...
	globalNonces[config.FirstUser] = 0 // FirstUser starts at 0 too

	// Start with genesis block
	blocks = append(blocks, blockchain.GenesisBlock)
	var prevHash [32]byte
	prevHash = hashing.HashBlockHeader(&blockchain.GenesisBlock.Header)

	// Block 1: Distribution block - FirstUser distributes coins to test accounts
	distributionTxs := make([]models.Transaction, 0, accountCount+1)
	...
	prevHash = hashing.HashBlockHeader(&distributionBlock.Header)

	// Generate remaining blocks with random transactions between funded accounts
	r := mathrand.New(mathrand.NewSource(time.Now().UnixNano()))
	
	for i := 0; i < blockCount; i++ {
		allTxs := make([]models.Transaction, 0, transactionsPerBlock)
		...
		prevHash = hashing.HashBlockHeader(&block.Header)
	}
...
func GenerateInvalidTransactions(accounts []TestAccount) map[string]models.Transaction {
	invalid := make(map[string]models.Transaction)
...
func GetSimpleTransferChain() []*models.Block {
	...
	block1 := GenerateMinedTestBlock([32]byte{}, []models.Transaction{coinbase1}, 1000)
	...
	block2 := GenerateMinedTestBlock(hashing.HashBlockHeader(&block1.Header), []models.Transaction{coinbase2, transfer1}, 2000)

	return []*models.Block{&block1, &block2}
}

// GetHighVolumeChain - For stress testing
func GetHighVolumeChain() []*models.Block {
...
func GetCircularTransferChain() []*models.Block {
	accounts := GenerateTestAccounts(3)
	...
	block1 := GenerateMinedTestBlock(prevHash, []models.Transaction{coinbase1, coinbase2, coinbase3}, 1000)
	blocks = append(blocks, &block1)
	prevHash = hashing.HashBlockHeader(&block1.Header)
	...
	block2 := GenerateMinedTestBlock(prevHash, []models.Transaction{coinbase, transfer1}, 2000)
	blocks = append(blocks, &block2)
	prevHash = hashing.HashBlockHeader(&block2.Header)
	...
	block3 := GenerateMinedTestBlock(prevHash, []models.Transaction{coinbase, transfer2}, 3000)
	blocks = append(blocks, &block3)
	prevHash = hashing.HashBlockHeader(&block3.Header)
	...
	block4 := GenerateMinedTestBlock(prevHash, []models.Transaction{coinbase, transfer3}, 4000)
...
func GetDoubleSpendChain() []*models.Block {
	accounts := GenerateTestAccounts(3)
	...
	coinbase1 := GenerateCoinbaseTransaction(PublicKeyToArray(accounts[0].PublicKey), 1000)
	block1 := GenerateMinedTestBlock(prevHash, []models.Transaction{coinbase1}, 1000)
	blocks = append(blocks, &block1)
	prevHash = hashing.HashBlockHeader(&block1.Header)
...
func GetInvalidNonceChain() []*models.Block {
